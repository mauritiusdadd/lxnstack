...
class AlignmentPoint(ImageFeature):
    ...
    def draw(self, painter):
        ...
        painter.drawText(rect,QtCore.Qt.AlignCenter,self.name)
        rect=Qt.QRectF(x-self.width/2,
                        y-self.height/2,
                        self.width,
                        self.height)
        painter.setBrush(QtCore.Qt.NoBrush)
...
class Star(ImageFeature):
    ...
    def draw(self, painter):
        ...
        painter.drawEllipse(Qt.QPointF(self.x,self.y),self.r1,self.r1)
        painter.drawEllipse(Qt.QPointF(self.x,self.y),self.r2,self.r2)
        painter.drawEllipse(Qt.QPointF(self.x,self.y),self.r3,self.r3)
        
        painter.setCompositionMode(0)
        
        if (self.name.strip() != '') and self.mouse_over:
            rect=Qt.QRectF(self.x+self.r3-2,self.y+self.r3-2,45,15)
            painter.setBrush(QtCore.Qt.blue)
            painter.setPen(QtCore.Qt.NoPen)
            painter.drawRect(rect)
