...
class MappedImage(QtCore.QObject):
    ...
    def __init__(self, data=None, cmap=colormaps.gray,
                 fit_levels=False, levels_range=(0, 100), name=''):
        ...
        self.levels_dlg = uic.loadUi(os.path.join(paths.UI_PATH,
                                                  'levels_dialog.ui'))

        # paint callback for histoGraphicsView
        self.levels_dlg.histoView.paintEvent = self.histoViewPaintEvent

        self.levels_dlg.curveTypeComboBox.currentIndexChanged.connect(
            self.updateHistograhm)
        self.levels_dlg.aDoubleSpinBox.valueChanged.connect(
            self.updateHistograhm2)
        self.levels_dlg.bDoubleSpinBox.valueChanged.connect(
            self.updateHistograhm2)
        self.levels_dlg.oDoubleSpinBox.valueChanged.connect(
            self.updateHistograhm2)
        self.levels_dlg.nDoubleSpinBox.valueChanged.connect(
            self.updateHistograhm2)
        self.levels_dlg.mDoubleSpinBox.valueChanged.connect(
            self.updateHistograhm2)
        self.levels_dlg.dataClippingGroupBox.toggled.connect(
            self.updateHistograhm2)
        self.levels_dlg.dataClipping8BitRadioButton.toggled.connect(
            self.updateHistograhm2)
        self.levels_dlg.dataClippingFitDataRadioButton.toggled.connect(
            self.updateHistograhm2)
        self.levels_dlg.MWBGroupBox.toggled.connect(
            self.updateHistograhm2)
        self.levels_dlg.histLogViewCheckBox.stateChanged.connect(
            self.updateHistograhm2)
        self.levels_dlg.buttonBox.clicked.connect(
            self.levelsDialogButtonBoxClickedEvent)
        self.levels_dlg.rComboBox.currentIndexChanged.connect(
            self.setComponentRed)
        self.levels_dlg.gComboBox.currentIndexChanged.connect(
            self.setComponentGreen)
        self.levels_dlg.bComboBox.currentIndexChanged.connect(
            self.setComponentBlue)
    ...
    def setData(self, data, update=True):
        ...
        # TODO:make a user's control?
        self._hst = utils.generateHistograhms(self._original_data, 128)
        self._original_hst = self._hst.copy()
    ...
    def getMappedData(self):
        if self._original_data is None:
            ...
            mapped_data = self._colormap.mapData(self.getData(),
                                                 self._fit_levels,
                                                 self._levels_range)
            arr = np.empty((self._original_data.shape[0],
    ...
    def remap(self):
        if self._original_data is not None:
            self._mapped_qimage = arrayToQImage(
                self.getData(),
                R=self.r_index,
                G=self.g_index,
                B=self.b_index,
    ...
    def signalMWBSlider(self, *arg, **args):
        if not self.__updating_mwb_ctrls:
            ...
            for name in self.component_ctrl_table:
                ...
                self.MWB_CORRECTION_FACTORS[name] = [
                    new_l_val,
                    new_m_val,
                    new_h_val]
    ...
    def getMWBCorrectionFactors(self):
        self.updateMWBCorrectionFactors()
        return (self.MWB_CORRECTION_FACTORS,
    ...
    def buildMWBControls(self):
        ...
        for i in self.component_table:
            ...
            self.component_ctrl_table[name] = (c_l_sld, c_l_dsb,
                                               c_m_sld, c_m_dsb,
                                               c_h_sld, c_h_dsb)

            log.log(self._logname,
                    "building controls for component "+str(name),
                    level=logging.DEBUG)
    ...
    def getData(self):
        ...
        if self.levelfunc_idx == 0:
            # linear
            newdata = A + B*data
        elif self.levelfunc_idx == 1:
            # logarithmic
            newdata = A + B*np.emath.logn(n, (o + m*data))
        elif self.levelfunc_idx == 2:
            # power
            newdata = A + B*((o + m*data)**n)
        elif self.levelfunc_idx == 3:
            # exponential
    ...
    def updateHistograhm(self, curve_idx):
        if self._ignore_histogrham_update or (self._hst is None):
            return
        use_clp = self.levels_dlg.dataClippingClipDataRadioButton.isChecked()
        use_fit = self.levels_dlg.dataClippingFitDataRadioButton.isChecked()
        dc_enabled = self.levels_dlg.dataClippingGroupBox.isChecked()

        clipping = dc_enabled and use_clp
        streching = dc_enabled and use_fit
        ...
        for i in range(len(self._hst)):
            ...
            if i > 0:
                if clipping:
                    data_mask = self._hst[i, 1]
                    mask = (data_mask >= data_min)*(data_mask <= data_max)
                ...
                for j in range(len(self._hst[i, 0])):
                    ...
                    try:
                        ...
                        self._hst[0, 0][k-1] += y*(x2-x) / delta
                        self._hst[0, 0][k] += y*(x-x1) / delta
                    except:
                        pass
...
def arrayToQImage(img, R=0, G=1, B=2, cmap=colormaps.gray,
                  fit_levels=False, levels_range=None):
    if img is None:
        return QtGui.QImage()
    elif type(img) != np.ndarray:
        raise TypeError('In module utils, in function arrayToQImage, ' +
                        'ndarray expected as first argumrnt but ' +
                        str(type(img))+' given instead')
    ...
    result = Qt.QImage(rawdata, optimal_w, optimal_h,
