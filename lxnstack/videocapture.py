...
#
# GENERIC CAPTURE INTERFACE (PARENT OBJECT OF ALL VIDEO CAPTURE INTERFACES)
#
    ...
    def requestLock(self):
        ...
        if utils.timeouted_loop(self.isLocked,timeout=1,t_step=0.1,c_val=False):
            ...
            if self._control_ui is not None:
                ...
                fps_menu = self._control_ui.findChild(QtGui.QComboBox,"framerate menu")
...
class V4l2VideoDevice(GenericVideoDevice):
    ...
    * http://linuxtv.org/downloads/v4l-dvb-apis/control.html
    ...
    def open(self, device=None,mode=None, pixfmt=None):
        ...
        by your device. You can obtain a list of supported methos by calling the
        function 'getAvailableReadingModes()'
        """
        ...
        if not self._reopening:    
            ...
            if (not (cap.capabilities & v4l2.V4L2_CAP_STREAMING)):
                ...
                can_stream=True
                self._available_reading_modes[self.MEMORY_MMAP]='userspace memory mapping'
                self._available_reading_modes[self.MEMORY_USERPTR]='userspace pointer'
            ...
            self._device_driver_version="{0:d}.{1:d}.{2:d}".format((cap.version>>16)&255,
                                                                (cap.version>>8)&255,
                                                                    cap.version&255)
    ...
    def isHighQualityDevice(self):
        ...
        For more information see:
            http://linuxtv.org/downloads/v4l-dvb-apis/vidioc-g-parm.html#parm-flags
        """
    ...
    def hasShutterSpeed(self):
        ...
        For more information see:
            http://linuxtv.org/downloads/v4l-dvb-apis/vidioc-g-parm.html#parm-caps
        """
    ...
    def setFrameRate(self,fps):
        ...
        ival_list=self._available_pixel_format[pixfmt]['framse_size'][(width,height)]
    ...
    def setFrameSize(self, width, height):
        ...
        if fmt is None:
            return False
        elif not ((width,height) in self._available_pixel_format[fmt.fmt.pix.pixelformat]['framse_size']):
            return False
        else:
    ...
    def setPixelFormat(self,pixfmt):
        ...
        if not (pixfmt in self._available_pixel_format):
            ...
            if (curr_w,curr_h) in self._available_pixel_format[pixfmt]['framse_size']:
                new_size=(curr_w,curr_h)
            else:
                try:
                    new_size = self._available_pixel_format[pixfmt]['framse_size'][0]
                except:
    ...
    def setFormat(self,width,height,pixfmt):        
        ...
        if not (pixfmt in self._available_pixel_format):
            return False
        elif not ((width,height) in self._available_pixel_format[pixfmt]['framse_size']):
            return False
    ...
    def _qt_ui_helper_update_frame_sizes(self,frm_menu):
        ...
        keys = self._available_pixel_format[self.getPixelFormat()]['framse_size'].keys()
        keys.sort()
        ...
        cidx=frm_menu.findData(QtCore.QVariant(QtCore.QSizeF(csize[0],csize[1])))
    ...
    def _qt_ui_helper_update_framerates(self,fps_menu):
        ...
        keys = self._available_pixel_format[self.getPixelFormat()]['framse_size'][self.getFrameSize()]
        keys.sort()
        keys.reverse()
        
        for item in keys:
            itemname=str(item[0])+' ('+str(item[1][0])+'/'+str(item[1][1])+') fps'
    ...
    def _qt_ui_helper_update_pixelformats(self,fmt_menu):
        ...
        for itemidx in self._available_pixel_format:
            ...
            if not (fccinfo in itemname.replace(' ','')):
                itemname+=' '+fccinfo
            ...
            for control in self._capture_controls[ctrl_class]:
                ...
                if ((control['type'] == v4l2.V4L2_CTRL_TYPE_INTEGER) or
                    ...
                    for itemidx in control['menuitems']:
                        qctrl1.insertItem(itemidx,str(control['menuitems'][itemidx]))
    ...
    def _ui_set_pixelformat(self,idx):
        ...
        self._qt_ui_helper_update_frame_sizes(self._control_ui.findChild(QtGui.QComboBox,"size menu"))
    ...
    def _ui_set_frame_size(self,idx):
        ...
        self._qt_ui_helper_update_framerates(self._control_ui.findChild(QtGui.QComboBox,"framerate menu"))
        self._refresh_ui_controls()
    ...
    def _xioctl(self, request, arg):
        ...
        if v4l2.HAS_LIBV4L2:
            while maxiter:
                maxiter-=1
                r = v4l2.v4l2_ioctl(ctypes.c_int8(self._fd), ctypes.c_uint32(request), ctypes.pointer(arg))
                eno = v4l2.v4l2_errno()
                if (r==-1):
            ...
            while maxiter:
                ...
                try:
                    ...
                    if (exc.errno == errno.EINTR) or (exc.errno == errno.EAGAIN):
                        time.sleep(0.005)
                    else:
                        return -1,exc.errno
            return -1,-1
    ...
    def _secure_xioctl(self, request, arg):
        ...
        was_streaming=self.isStreaming()
        if was_streaming:
            #NOTE: In order to change some parameters, we must first
            #       disable the video streaming...
    ...
    def _enumerate_controls(self):
        ...
        if r>=0:
            ...
            for queryctrl.id in xrange(v4l2.V4L2_CID_BASE,v4l2.V4L2_CID_LASTP1+1):
                r,err = self._xioctl(v4l2.VIDIOC_QUERYCTRL, queryctrl)
                if r == -1:
                    if err == errno.EINVAL:
    ...
    def _set_capture_param(self,frmival=None,hiq=None,override_parm=None):
        ...
        was_streaming=self.isStreaming()
        self._reopening=True
        if not(self.close() and self.open(self._device_filename, self._io,fmt)):
            self._reopening=False
            return False
    ...
    def _enum_frame_intervals(self,pixfmt,width,height):
        ...
        self._available_pixel_format[pixfmt]['framse_size'][(width,height)]=[]
        ...
        while 1:
            ...
            if frmival.type == v4l2.V4L2_FRMIVAL_TYPE_DISCRETE:
                ...
                self._available_pixel_format[pixfmt]['framse_size'][(width,height)].append((fps,(i_num,i_den)))
                frmival.index+=1
                ...
                self._available_pixel_format[pixfmt]['framse_size'][(width,height)].append((i_min_fps,(i_min_num,i_min_den)))
                ...
                while val <= max_i:
                    ...
                    val=float(n)/float(d)
                    fps=float(d)/float(n)
                    self._available_pixel_format[pixfmt]['framse_size'][(width,height)].append((fps,(n,d)))
    ...
    def _enum_frame_size(self,pixfmt):
        ...
        self._available_pixel_format[pixfmt]['framse_size']={}
        ...
        while 1:
            ...
            if frmsize.type == v4l2.V4L2_FRMSIZE_TYPE_DISCRETE:
                ...
                self._available_pixel_format[pixfmt]['framse_size'][(w,h)]=None
                ...
                if not max_h in h_list:
                    h_list+=[max_h]
                
                # I'm not sure how to handle this case:
                # assuming that width and height are pairs and 
                # are not indipendet values
                for wxh in zip(w,h):
                    self._available_pixel_format[pixfmt]['framse_size'][(w,h)]=None
                    self._enum_frame_intervals(pixfmt,w,h)
                break
    ...
    def _enum_pixel_format(self):
        ...
        while 1:
            ...
            self._available_pixel_format[fmtdesc.pixelformat]={'fourcc'     : fourcc,
    ...
    def _set_pixel_format(self ,wid=None, hei=None, 
        ...
        if override is None:
            ...
            if (wid is None) and (hei is None):
                try:
                    wid,hei = self._available_pixel_format[pixfmt]['framse_size'].keys()[-1]
                except Exception as exc:
                    return None
            ...
            if preserve:
                ...
                if self._format is None:
                    ...
                    if fmt.fmt.pix.pixelformat not in self.supported_pixel_formats:
                        fmt.fmt.pix.pixelformat = pixfmt
    ...
    def _streamon(self):
        if self._io == self.MEMORY_READ:
            ...
            return True
        elif (-1 == self._xioctl(v4l2.VIDIOC_STREAMON, v4l2.v4l2_buf_type(v4l2.V4L2_BUF_TYPE_VIDEO_CAPTURE))[0]):
    ...
    def _streamoff(self):
        if self._io == self.MEMORY_READ:
            ...
            return True        
        if (-1 == self._xioctl(v4l2.VIDIOC_STREAMOFF, v4l2.v4l2_buf_type(v4l2.V4L2_BUF_TYPE_VIDEO_CAPTURE))[0]):
    ...
    def _init_read(self):
        ...
        try:
            ...
            return False
        self._buffers={0:{'start':ctypes.addressof(sbuf),'length':buffer_size,'arr':sbuf}}
        return True
    ...
    def _init_mmap(self):
        ...
        for n_buffers in range(req.count):
            ...
            try:
                ...
                if v4l2.HAS_LIBV4L2:
                    buff_start = v4l2.v4l2_mmap(0, buf.length,
                    ...
                    buff_start =  mmap.mmap(self._fd,
                                            buf.length,
                                            prot=(mmap.PROT_READ | mmap.PROT_WRITE),
                                            access=mmap.MAP_SHARED,
                                            offset=buf.m.offset)
                    
                self._buffers[n_buffers] = {'start':buff_start,'length':buff_length}
    ...
    def read_frame(self):
        ...
        if self._io == self.MEMORY_READ:
            r = v4l2.v4l2_read(self._fd, self._buffers[0]['start'], self._buffers[0]['length'])
            if (-1 == r):
    ...
    def _process_image(self,buf):
        ...
        #n = self._format.pix.bytesperline/w #this is true only if we have 8 bit per pixel!
        try:
            if  self._format.pix.pixelformat == v4l2.V4L2_PIX_FMT_RGB444:
                rawarr = np.ndarray((h,w),dtype=np.uint16,buffer=sbuf.raw,order='C')
                ...
                arr = np.ndarray((h,w,3),dtype=np.uint8,buffer=sbuf.raw,order='C')
            
            elif self._format.pix.pixelformat == v4l2.V4L2_PIX_FMT_RGB32:            
                arr = np.ndarray((h,w,4),dtype=np.uint8,buffer=sbuf.raw,order='C')
                ...
                arr = np.ndarray((h,w),dtype=np.uint8,buffer=sbuf.raw,order='C')
                
            elif self._format.pix.pixelformat == v4l2.V4L2_PIX_FMT_Y16:
                arr = np.ndarray((h,w),dtype=np.uint16,buffer=sbuf.raw,order='C')            
            else:
...
class Cv2VideoDevice(GenericVideoDevice):
            ...
            if (not os.path.exists(device) and
                ...
                if (len(dev_file)>5) and (dev_file[:5]=="video"):
                    try:
                        ...
                        raise IOError("Cannot open the device \"" + filename + "\"")
    ...
    def _qt_ui_helper_update_framerates(self,fps_menu):
        ...
        keys = self._available_pixel_format[self.getPixelFormat()][self.getFrameSize()]
        keys.sort()
        keys.reverse()
    ...
    def _qt_build_ui_controls_interface(self):
        ...
        if 'formats' in self._capture_controls:
            self._available_pixel_format=self._capture_controls.pop('formats')
        else:
            self._available_pixel_format={}
        ...
        props=getV4L2DeviceProperties(self._filename)
    ...
    def setFrameSize(self, width, height):
        ...
        if not setV4L2Format(self._filename,'width='+str(width)+',height='+str(height)):
            return False
            
        return (width,height) == self.getFrameSize()
        
    def getFrameRate(self):
        val = self._device.get(cv2.cv.CV_CAP_PROP_FPS) # *very* often this function is broken!
        
        if val > 0:
            return val
        
        try:
            v4l2_ctl = subprocess.Popen(['v4l2-ctl','--device='+self._filename, '--get-parm'], stdout=subprocess.PIPE)
            v4l2_ctl.wait()
    ...
    def setFrameRate(self, fps):
        # *very* often this function is broken!
        if self._device.set(cv2.cv.CV_CAP_PROP_FPS, fps):
            return True
                    
        try:
            v4l2_ctl = subprocess.Popen(['v4l2-ctl','--device='+self._filename, '--set-parm='+str(fps)], stdout=subprocess.PIPE)
            v4l2_ctl.wait()
            rawdata=v4l2_ctl.stdout.read().replace(' ','').replace('\t','')
        except:
            return False
...
class CaptureJob(Qt.QObject):
    ...
    def start(self):
        if self._status!=self.StatusInProgress:
            ...
            self._capture_thread.started.connect(self.__threaded_video_capturing)
    ...
    def __threaded_video_capturing(self):
        ...
        if self._device.requestLock():
            ...
            if self.type == self.TypeVideo:
                hexcnt=0
                file_name = os.path.join(self.getDestinationDir(), str(self.name)+"-{0:04x}.".format(hexcnt)+self._output_file_type)
                ...
                hexcnt=0
                dir_name = os.path.join(self.getDestinationDir(), str(self.name)+"-{0:04x}".format(hexcnt))
                ...
                while(self._status == self.StatusInProgress):
                    file_name=os.path.join(dir_name,"{0:08x}".format(captured_frames))
        ...
        self.moveToThread(QtGui.QApplication.instance().thread()) #back to main thread
...
class CaptureScheduler(Qt.QObject):
    ...
    def __init__(self, capture_device=None):
        Qt.QObject.__init__(self)
        self.jobs={}  
        self._controlgui = uic.loadUi(os.path.join(paths.UI_PATH,'sched_dialog.ui'))
        ...
        self.jobTimeView.__paintEvent__= self.jobTimeView.paintEvent #base implementation
        self.jobTimeView.paintEvent = self.jobTimeViewPaintEvent #new callback        
        ...
        self._controlgui.addJobPushButton.setIcon(utils.getQIcon("document-new"))
        self._controlgui.deleteJobPushButton.setIcon(utils.getQIcon("edit-delete"))
        self._controlgui.deleteAllJobsPushButton.setIcon(utils.getQIcon("edit-delete"))
        self._controlgui.confirmPushButton.setIcon(utils.getQIcon("ok"))
        self._controlgui.closePushButton.setIcon(utils.getQIcon("window-close"))
        
        self._controlgui.jobListWidget.currentRowChanged.connect(self.refreshJobInfo)
        self._controlgui.addJobPushButton.clicked.connect(self.addJob)
        self._controlgui.deleteJobPushButton.clicked.connect(self.deleteJob)
        self._controlgui.deleteAllJobsPushButton.clicked.connect(self.deleteAllJobs)
        self._controlgui.confirmPushButton.clicked.connect(self._choosEditeAction)
        #self._controlgui.startAllJobPushButton
        #self._controlgui.stopAllJobPushButton
        
        self._controlgui.startTimeNowRadioButton.toggled.connect(self.updateCurrentJobStartType)
        self._controlgui.startTimeAtRadioButton.toggled.connect(self.updateCurrentJobStartType)
        
        self._controlgui.endTimeDurationRadioButton.toggled.connect(self.updateCurrentJobEndType)
        self._controlgui.endTimeAtRadioButton.toggled.connect(self.updateCurrentJobEndType)
        self._controlgui.endTimeFramesRadioButton.toggled.connect(self.updateCurrentJobEndType)
        
        self._controlgui.captureTypeComboBox.currentIndexChanged.connect(self.updateCurrentJobType)
        self._controlgui.startDateTimeEdit.dateTimeChanged.connect(self.updateCurrentJobStartTime)
        self._controlgui.endDateTimeEdit.dateTimeChanged.connect(self.updateCurrentJobEndTime)
        self._controlgui.durationDoubleSpinBox.valueChanged.connect(self.updateCurrentJobDuration)
        self._controlgui.delayDoubleSpinBox.valueChanged.connect(self.updateCurrentJobDelay)
        
        self._timer.timeout.connect(self._updateAll)
        self._timer.start()
    
    def jobTimeViewPaintEvent(self, obj):
        val=self.jobTimeView.__paintEvent__(obj)
        ...
        for x0 in xrange(-xoff,wmax+int(bigstep),step):
            ...
            if x0%bigstep == 0:
                ...
                painter.drawText(x-w0,0,2*w0,y0,QtCore.Qt.AlignCenter,txt)
        ...
        for job in self.jobs.values():
            ...
            if status == CaptureJob.StatusInactive:
                #Waiting the for the user fish the editing and activate the job
            ...
            painter.drawText(rect,QtCore.Qt.AlignCenter,job.name+"\n"+job._status_text)
    ...
    def addJob(self, job=None, jobid=None):
        ...
        joblistwidgetitem = Qt.QListWidgetItem(itemname,self._controlgui.jobListWidget)
    ...
    def refreshJobInfo(self, row):
        ...
        if joblistwidgetitem_curr is not None:
            ...
            if joblistwidgetitem_prev is not None:
                ...
                self._controlgui.startDateTimeEdit.setMinimumDateTime(min_start_date)
                del min_start_date
            else:
            ...
            if joblistwidgetitem_next is not None:
                ...
                self._controlgui.endDateTimeEdit.setMaximumDateTime(max_end_date)
                del max_end_date
            else:
    ...
    def _updateAll(self,*args):
        ...
        self._controlgui.deleteAllJobsPushButton.setEnabled((len(self.jobs)>0) and not self._global_status)
            ...
            if joblistwidgetitem is not None:
                ...
                if status == CaptureJob.StatusInactive:
                    ...
                    if job._start_type==0:
                        ...
                        t_now=Qt.QDateTime.currentDateTime().toMSecsSinceEpoch()+delay
                        
                        if joblistwidgetitem_prev is not None:
                            t_last=self.getJob(joblistwidgetitem_prev.jobid).getEndTime()+delay
                            if t_now > t_last:
                    ...
                    count=(utils.getCurrentTimeMsec() - job.getStartTime())
                    status_txt=utils.tr("In progress")+" {0:0.02f}sec".format(count/1000.0)
                    if job._end_type == 2:
                        if job.getNumberOfFrames() < 0:
                            pass # this means that the job will never end uless manually stopped
                    elif utils.getCurrentTimeMsec() >= job.getEndTime():
                        #job._status=CaptureJob.StatusDone
                        job.stop()
                                                            
                elif status == CaptureJob.StatusWaiting:
                    #Job is waiting until the start time is reached
                    joblistwidgetitem.setBackground(QtCore.Qt.white)
                    countdown=(utils.getCurrentTimeMsec() - job.getStartTime())/1000.0
                    status_txt=utils.tr("Waiting")+" {0:0.02f} sec".format(countdown)
                    if utils.getCurrentTimeMsec() >= job.getStartTime():
                        #job._status=CaptureJob.StatusInProgress
                        job.start()
                elif status == CaptureJob.StatusError:
                ...
                itemname = "[ {5} ]\tid: \"{0}\" duration:{2:0.02f}  delay:{1:0.02f}  start: {3}  stop: {4}".format(jobid,
                           job.getDelay()/1000.0, (end_time-start_time)/1000.0,
        ...
        ww = int((_maxtime-utils.getCurrentTimeMsec() + self._past_view + 1000)*self._timescale)
    ...
    def updateCurrentJobType(self,idx):
        ...
        self._current_job.setType(self._controlgui.captureTypeComboBox.currentIndex())        
    ...
    def updateCurrentJobStartTime(self,start_date):
        ...
        if self._controlgui.endTimeDurationRadioButton.isChecked():
            ...
            end_time = self._controlgui.endDateTimeEdit.dateTime().toMSecsSinceEpoch()
    ...
    def updateCurrentJobEndTime(self,end_date):
        ...
        self._refreshing=True
        start_time = self._controlgui.startDateTimeEdit.dateTime().toMSecsSinceEpoch()
        end_time = end_date.toMSecsSinceEpoch()
        duration = end_time - start_time
        
        self._current_job.setEndTime(end_date)
        #max_start_date=Qt.QDateTime()
        #max_start_date.setMSecsSinceEpoch(end_time-1)
        
        #self._controlgui.startDateTimeEdit.setMaximumDateTime(max_start_date)
        self._controlgui.durationDoubleSpinBox.setValue(duration/1000.0)
    ...
    def _updateListWidget(self):
        if len(self.jobs) != self._controlgui.jobListWidget.count():
            ...
            showErrorMsgBox(utils.tr("Somethig terrible wrong has happend!\nPlease send a bug report to the author."))
...
def getV4L2Format(device):
    fmt={}
    try:
        v4l2_ctl = subprocess.Popen(['v4l2-ctl','--device='+device, '--get-fmt-video'],stdout=subprocess.PIPE)
        v4l2_ctl.wait()
        rawdata=v4l2_ctl.stdout.read().replace(' ','').replace('\t','')
    except Exception as exc:
        return {}
...
def getV4L2Ctrl(device,ctrl):
    try:
        v4l2_ctl = subprocess.Popen(['v4l2-ctl','--device='+device, '--get-ctrl='+ctrl],stdout=subprocess.PIPE)
        v4l2_ctl.wait()
        rawdata=v4l2_ctl.stdout.read().replace(' ','').replace('\t','')
    except:
        return None
...
def getV4L2DeviceProperties(device):
    ...
    for line in rawdata:
        if ' : ' in line:
            ...
            props[name]={'min':None, 'max':None, 'default':None, 'value':None, 'flags':None, 'step':1}
